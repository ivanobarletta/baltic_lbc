...
def calcNEMOTransport(                                  # Wrapper to the main function
    ...
    # load coordinates of mesh F-points
    ...
    if makePlot:
        ...
        ax.scatter(dsCoords["glamu"],dsCoords["gphiu"],transform=ccrs.PlateCarree(),color="g",s=0.2)    # u-points
        ax.scatter(dsCoords["glamv"],dsCoords["gphiv"],transform=ccrs.PlateCarree(),color="r",s=0.2)    # v-points
...
def calcXFaces(
    ...
    xFacesDa = xr.DataArray(xFaces,dims=["Z","v-points"])
...
def calcYFaces(
    ...
    yFacesDa = xr.DataArray(yFaces,dims=["Z","u-points"])
