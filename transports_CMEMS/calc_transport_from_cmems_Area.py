...
"""
    ...
    3) Area info
        ...
        areaCoords = [lon1,lon2,lat1,lat2]
...
def calc_x_transport_single_file(
    ...
    if pathS != None:
        so  = so.sel(longitude=slice(xArea1,xArea2),latitude=slice(yArea1,yArea2))           #(t,z,y)
        if dates != None:
            print("time slice")
    ...
    if pathT != None:
        ...
        thetao  = thetao.sel(longitude=slice(xArea1,xArea2),latitude=slice(yArea1,yArea2))   #(t,z,y)

    uo  = uo.sel(longitude=slice(xArea1,xArea2),latitude=slice(yArea1,yArea2))   #(t,z,y)
    ssh = ssh.sel(longitude=slice(xArea1,xArea2),latitude=slice(yArea1,yArea2))  #(t,y)
    e2t = e2t.sel(longitude=slice(xArea1,xArea2),latitude=slice(yArea1,yArea2))  #(y)
    e3t = e3t.sel(longitude=slice(xArea1,xArea2),latitude=slice(yArea1,yArea2))  #(z,y)
    H   = H.sel(longitude=slice(xArea1,xArea2),latitude=slice(yArea1,yArea2))    #(y)
    ...
    xfaces = Jacobian * ( e3t * e2t )                                               # (time,depth,latitude,longitude) 
    ...
    outFile = "transports_%s_area_%s.nc" % (parentModel, areaName)
    ...
    dsOut   = dsOut.assign_attrs({"areaName":areaName,"areaLon1":areaCoords[0],"areaLon2":areaCoords[1],"areaLat1":areaCoords[2],"areaLat2":areaCoords[3]})
...
areaCoords  = [13.35,13.60,54.4,55.45]
areaName    = "arkona"
