...
nemo_path = "cmems_glob0.083/%s/*nc" % year_month

transect_path = "neatl/NEATL36_obcdta_east_2_20220115P01_R20220123.nc"

ds = xr.open_mfdataset(nemo_path)
...
transect = ds[varname].isel(time=0).interp(longitude=x_int,latitude=y_int)
...
plt.title(title)
plt.savefig("cmems_glob0.083/%s/%s_transect_%s" % (year_month,varname,date_str))
