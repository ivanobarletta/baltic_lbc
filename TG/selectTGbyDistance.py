...
outFile             = "inventoryTG_Filtered.csv"
...
# create coordinates to make query
...
query_points = np.vstack((nav_lon.data.flatten(),nav_lat.data.flatten())).T
# query points is an array with shape (N,2)
# where N is len(nav_lon.data.flatten())

# make query
dist,idx    = tree.query(query_points,k=1)

# reshape distances to (ny,nx)
dist2D      = np.reshape(dist,nav_lon.shape)
"""
