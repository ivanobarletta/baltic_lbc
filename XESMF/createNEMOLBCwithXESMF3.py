...
if len(sys.argv) < 4:
    print("Too few arguments! Provide:")
    print("1) date (YYYYMMDD)")
    print("2) file3D path")
    print("3) fileSSH path")
    sys.exit(0)
...
angles_path = "../static_files/pyMakeAngles/pyangles_NEATL36_east2.nc"
angles_path = "../static_files/makeAngles/neatl36_angles_east2.nc"
method      = "conservative"
outFolder   = "outputs_bal"
outPath     = outFolder + "/" + "NEATL36_east2_XESMF_BAL_%s.nc" % dateYMD
...
def main(dateYMD=dateYMD,
    ...
    cdo = Cdo()
    dsFill = cdo.vertfillmiss(input=dsFill,returnXDataset=True)
    if l_dbg:
        dsFill.to_netcdf("dsVFill.nc")
    ...
    dsOut = rename_dataset(dsOut,dateYMD)
    ...
    dsOut.to_netcdf(outFile,unlimited_dims="T")
